...
<cfcomponent displayname= "DataTableObject"
	...
	<cffunction name= "toJSON"
		...
		return paginatedResultToJSON( qResult );
		</cfscript>
	...
	<cffunction name= "paginatedResultToJSON"
		...
		lsColumnNames= pqResult.columnList;
...
<cfsavecontent variable= "sJSON"><cfoutput>{
	...
	"aaData": [
<cfloop query= "pqResult"><cfsilent>
	...
	<cfset bLastColumn= false />
			...
			<cfif iColumnPosition && iColumnPosition neq -1>
				<cfset	sTempValue=	"" & pqResult[ sColumnName ][ currentrow ] />
			</cfif>
</cfsilent>
			<cfif iColumnPosition><cfif iColumnPosition eq -1>""<cfelse>"#sTempValue.ReplaceAll( '(["\\])' , "\\$1" ).ReplaceAll( "\t" , " " ).ReplaceAll( "(?m)(?s)[\n\r]" , "" )#"</cfif><cfelse>""</cfif><cfsilent>
